...
import 'package:uber_eats_consumer/ui/chat/PlayerWidget.dart';
import 'package:uber_eats_consumer/ui/fullScreenImageViewer/FullScreenImageViewer.dart';
import 'package:uber_eats_consumer/ui/fullScreenVideoViewer/FullScreenVideoViewer.dart';
                        ...
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          if (snapshot.hasData && (snapshot.data?.message.isEmpty ?? true)) {
                            return Padding(
                              ...
                              child:
                                  Center(child: showEmptyState('No Messages Yet'.tr(), 'Send a new message and it will show up here'.tr())),
                            );
                          } else {
        ...
        CupertinoActionSheetAction(
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              Navigator.pop(context);
              _showAlertDialog(
                  context, 'Report'.tr(), "hasblocked&reported".tr(args: ['${homeConversationModel.members.first.fullName()}']));
            } else {
              _showAlertDialog(
                  context, 'Report'.tr(), "Couldnotreport&block".tr(args: ['${homeConversationModel.members.first.fullName()}']));
            }
          },
        ),
      ],
