...
class _PlaceOrderScreenState extends State<PlaceOrderScreen> {
        ...
        children: [
          ListTile(
            title: Text(
              'Placing Order...'.tr(),
              style: TextStyle(
                  color: isDarkMode(context) ? Colors.grey.shade300 : Colors.grey.shade800, fontSize: 20, fontWeight: FontWeight.bold),
            ),
            trailing: Container(
              width: 24,
              height: 24,
  ...
  animateOut() {
    ...
    FireStoreUtils.sendFcmMessage(
        "newOrder".tr(), '${MyAppState.currentUser!.firstName}' + "hasOrdered", widget.orderModel.vendor.fcmToken, ORDERREQUEST);
