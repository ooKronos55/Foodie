...
class _TableOrderDetailsScreenState extends State<TableOrderDetailsScreen> {
  ...
  void initState() {
    ...
    if (widget.bookTableModel.status == ORDER_STATUS_PLACED) {
      ...
      polyLinesFuture = polylinePoints.getRouteBetweenCoordinates(
          GOOGLE_API_KEY,
          PointLatLng(widget.bookTableModel.vendor.latitude, widget.bookTableModel.vendor.longitude),
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          InkWell(
                            onTap: () {
                              launchUrl(createCoordinatesUrl(widget.bookTableModel.vendor.latitude, widget.bookTableModel.vendor.longitude,
                                  widget.bookTableModel.vendor.title));
                            },
                          ...
                          InkWell(
                            onTap: () {
                              if (widget.bookTableModel.vendor.phonenumber != null && widget.bookTableModel.vendor.phonenumber.isNotEmpty) {
  ...
  void _onMapCreated(GoogleMapController controller, BookTableModel orderModel) {
    ...
    if (orderModel.status == ORDER_STATUS_IN_TRANSIT) {
      updateCameraLocation(LatLng(orderModel.vendor.latitude, orderModel.vendor.longitude),
          LatLng(orderModel.author.location.latitude, orderModel.author.location.longitude), _mapController);
    }
  }
